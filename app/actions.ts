...
export async function claimIdentity(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  const existing = await sql`select id, pin_hash from gambit.users where name_lower=${nameLower}`;
  ...
  if (existing[0]) {
    ...
    const [row] = await sql`
      ...
      values (${name}, ${nameLower}, ${hashPin(pin)}) returning id`;
    userId = n(row.id);
...
export async function placeFutures(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      const [p] = await tx`select name, eliminated from gambit.players where id=${playerId}`;
...
export async function adminSetStatus(formData: FormData) {
  ...
  if (status !== "open" && status !== "locked") throw new Error("Bad status.");
...
export async function adminSettle(formData: FormData) {
  const admin = await getSessionUser();
  if (!admin?.is_admin) throw new Error("Not authorised.");
  const matchId = String(formData.get("matchId") ?? "");
